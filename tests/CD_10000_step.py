...
f = open('CD_10000_STEP.csv', 'w')#file to write chain data in
print('Loading...')
pdb = PDBFile('CD.pdb')
...
simulation.reporters.append(StateDataReporter(f, 10000, step=True,kineticEnergy=True,
                                              potentialEnergy=True,totalEnergy = True,temperature=True,density = True))
#for every 100 steps, write into f 
simulation.step(120000)#run for x steps
print('done')
